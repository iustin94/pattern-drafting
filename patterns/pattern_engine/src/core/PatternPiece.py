...
from patterns.pattern_engine.src.core.Curve import Curve, CurveWithPeak, CurveWithReference
        ...
        for path in self.paths:
            ...
            for segment in path:
                if isinstance(segment, Line):
                    ...
                    if isinstance(segment, CurveWithPeak):
                        mirrored_path.append(CurveWithPeak(
                            start, end, segment.peak_value, segment.inflection_point
                        ))
                    elif isinstance(segment, CurveWithReference):
                        ref = mirrored.get_point(self._find_point_name(segment.reference_point))
                        mirrored_path.append(CurveWithReference(
